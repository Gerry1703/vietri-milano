...
export default function Footer() {
  return (
    <footer className="bg-beige-warm text-beige-light py-16 md:py-20 px-6 md:px-10">
      {/* Wordmark */}
      <div className="text-center mb-14">
        <p className="font-cormorant font-light text-beige-light tracking-widest4 text-4xl md:text-5xl uppercase select-none">
          VIETRI
        </p>
      </div>
      ...
      <div className="max-w-screen-lg mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8 text-center md:text-left mb-14">
        <div>
          ...
          <p className="font-inter font-light text-beige-light/60 text-sm leading-loose">
        ...
        <div>
          ...
          <nav className="flex flex-col gap-2">
            {[['Sciarpe', '/collection?cat=sciarpe'], ['Borse', '/collection?cat=borse'], ['Novità', '/collection']].map(([l, to]) => (
              <Link key={l} to={to} className="font-inter font-light text-beige-light/60 hover:text-beige-light text-sm transition-colors duration-300">
                {l}
              </Link>
            ))}
          </nav>
      ...
      <div className="max-w-screen-lg mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-2">
          <p className="label-upper text-beige-light/30 text-[10px] tracking-widest2">
            © 2025 VIETRI Milano
          </p>
          <p className="label-upper text-beige-light/30 text-[10px] tracking-widest2">
            P.IVA XXXXXXXX
